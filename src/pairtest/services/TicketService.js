...
export default class TicketService {
  ...
  #calculateAmountToPay(ticketTypeRequests) {
    return ticketTypeRequests.reduce((acc, ticketReq) => {
      return (
        acc +
        ticketRepository.getPrice(ticketReq.getTicketType()) *
          ticketReq.getNoOfTickets()
      );
    }, 0);
  }
  ...
  purchaseTickets(accountId, ticketTypeRequests) {
    if (
      this.#calculateTotalTicketAmount(ticketTypeRequests) > MAXIMUM_TICKETS
    ) {
      const err = new InvalidPurchaseException(
        `Cannot purchase more than ${MAXIMUM_TICKETS} tickets at a time`
      );
      logger.error(err);
      throw err;
    }
    ...
    if (!this.#hasAdultTicket(ticketTypeRequests)) {
      const err = new InvalidPurchaseException(
        "Ticket purchases require at least 1 adult"
      );
      logger.error(err);
      throw err;
    }
    ...
    try {
      ...
      const totalSeatsToAllocate =
        this.#calculateTotalSeats(ticketTypeRequests);
      ...
      logger.info(
        `Comleted payment and seat reservations for id: ${accountId}`
      );
      ...
      logger.error(err);
      throw new InternalPurchaseException(
        "An internal error occurred when purchasing tickets"
      );
    }
  }
}
